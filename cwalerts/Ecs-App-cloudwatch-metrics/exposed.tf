resource "aws_cloudwatch_metric_alarm" "DesiredTaskCount" {
  count               = length(var.exposedservices)
  alarm_name          = "${var.name_prefix}-DesiredTaskCount-${var.exposedservices[count.index]["taskdef_name"]}"
  ...
  dimensions={
    ...
    ServiceName = var.exposedservices[count.index]["taskdef_name"]
  }

  alarm_actions     = ["${aws_sns_topic.ecs.arn}"]
  depends_on = [
    aws_ecs_cluster.cluster,
    aws_ecs_service.ecs-service-exposed,
  ]
}

resource "aws_cloudwatch_metric_alarm" "memory_utilization_high" {
  count               = length(var.exposedservices)
  alarm_name          = "${var.name_prefix}-High-memory-${var.exposedservices[count.index]["taskdef_name"]}"
...
resource "aws_cloudwatch_metric_alarm" "cpu_utilization_high" {
  count               = length(var.exposedservices)
  alarm_name          = "${var.name_prefix}-Cpu-high-${var.exposedservices[count.index]["taskdef_name"]}"
...
resource "aws_cloudwatch_metric_alarm" "cpu_reservation_high" {
  count               = length(var.exposedservices)
  alarm_name          = "${var.name_prefix}-CPU_Reservation_high-${var.exposedservices[count.index]["taskdef_name"]}"
...
resource "aws_cloudwatch_metric_alarm" "cpu_reservation_low" {
  count               = length(var.exposedservices)
  alarm_name          = "${var.name_prefix}-Cpu_reservation_low-${var.exposedservices[count.index]["taskdef_name"]}"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = var.evaluation_periods
  metric_name         = "CPUReserved"
  namespace           = "AWS/ContainerInsights"
...
resource "aws_cloudwatch_metric_alarm" "memory_reservation_high" {
  count               = length(var.exposedservices)
  alarm_name          = "${var.name_prefix}-Memory_reservation-high-${var.exposedservices[count.index]["taskdef_name"]}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = var.evaluation_periods
...
resource "aws_cloudwatch_metric_alarm" "Service_count" {
  count               = length(var.exposedservices)
  alarm_name          = "${var.name_prefix}-Service-count-${var.exposedservices[count.index]["taskdef_name"]}"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = var.evaluation_periods
...
resource "aws_cloudwatch_metric_alarm" "Task_count" {
  count               = length(var.exposedservices)
  alarm_name          = "${var.name_prefix}-Task-count-${var.exposedservices[count.index]["taskdef_name"]}"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = var.evaluation_periods
...
resource "aws_cloudwatch_metric_alarm" "Running_Task_count" {
  count               = length(var.exposedservices)
  alarm_name          = "Running_Task_count-${var.exposedservices[count.index]["taskdef_name"]}"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = var.evaluation_periods
