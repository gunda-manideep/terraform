...
# Make a topic
resource "aws_sns_topic" "insights_alarm" {
...
depends_on = [aws_sns_topic.insights_alarm]
count      =length(var.emails)
  provisioner "local-exec" {
    command = "aws sns subscribe --topic-arn ${aws_sns_topic.insights_alarm.arn} --protocol email --notification-endpoint ${element(var.emails, count.index)}"
  }
}
...
data "aws_iam_policy_document" "sns_topic_policy" {
  ...
  statement {
    ...
    effect    = "Allow"
    resources = ["${aws_sns_topic.insights_alarm.arn}"]
